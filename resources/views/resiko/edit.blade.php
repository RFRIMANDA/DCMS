...
<div class="container">
   ...
   @endif
    ...
    <form action="{{ route('resiko.update', $resiko->id) }}" method="POST">
        ...
        <!-- Nama Resiko -->
        <div class="row mb-3">
            <span class="badge bg-info" style="font-size: 1.25rem;">TINGKATAN (Matriks Before)</span>
            <label for="nama_resiko" class="col-sm-2 col-form-label"><strong>Risiko</strong></label>
            <div class="col-sm-10">
                <textarea name="nama_resiko" class="form-control" rows="3" required>{{ old('nama_resiko', $resiko->nama_resiko) }}</textarea>
            </div>
        </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-4">
                <select class="form-select" name="probability" id="probability" onchange="calculateTingkatan()" required>
        ...
        <div class="row mb-3">
        <span class="badge bg-info" style="font-size: 1.25rem;">ACTUAL RISK (Matriks After)</span>
        ...
        <!-- Probability Risk -->
        <div class="row mb-3">
            <label for="probabilityrisk" class="col-sm-2 col-form-label"><strong>Probability / Kemungkinan Terjadi</strong></label>
            <div class="col-sm-4">
        ...
        <script>
            function validateRiskFields() {
                const severity = document.getElementById('severityrisk').value;
                const probability = document.getElementById('probabilityrisk').value;

                if ((severity && !probability) || (!severity && probability)) {
                    alert('Harap isi kedua field: Severity dan Probability Matriks After.');
                    return false;
                }
            ...
            // Tambahkan validasi ini saat form disubmit
            document.querySelector('form').addEventListener('submit', function (e) {
                if (!validateRiskFields()) {
