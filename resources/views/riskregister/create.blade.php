...
<form action="{{ route('riskregister.store') }}" method="POST">
    ...
    <div class="card">
    <span class="badge bg-info" style="font-size: 1.25rem;">Create Risk & Opportunity Register </span>
    </div>
    ...
    <div class="col-sm-10">
        <div class="dropdown">
            ...
            <ul class="dropdown-menu checkbox-group" aria-labelledby="dropdownDivisiAkses" style="max-height: 200px; overflow-y: auto;">
                <li class="px-2 py-1">
                    <input type="text" id="searchPihak" class="form-control form-control-sm" placeholder="Cari...">
                </li>
                <li>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const searchInput = document.getElementById('searchPihak');
        const allCheckboxItems = document.querySelectorAll('.checkbox-group li');
        ...
        searchInput.addEventListener('input', function () {
            ...
            allCheckboxItems.forEach(item => {
                ...
                if (label && !label.htmlFor.includes('select-all') && !label.htmlFor.includes('otherCheckbox')) {
...
<script>
    document.querySelector("form").addEventListener("submit", function(e) {
        const checkboxes = document.querySelectorAll('input[name="pihak[]"]');
        const otherChecked = document.getElementById("otherCheckbox").checked;
        const otherInput = document.getElementById("pihakOther").value.trim();

        let isChecked = false;

        // Cek apakah minimal satu checkbox pihak dicentang
        ...
        if (!isChecked) {
            e.preventDefault();
            alert("Silakan pilih minimal satu pihak berkepentingan atau isi opsi 'Other'.");
        }
    });
    </script>
...
<script>
    ...
    function updateDropdown() {
        ...
        // Ambil label dari checkbox yang dicentang (kecuali Select All & Other)
        checkboxes.forEach(checkbox => {
        ...
        // Tambahkan nilai input "Other" jika ada
        if (otherCheckbox.checked && otherInput.value.trim() !== '') {
            selectedValues.push(otherInput.value.trim());
        }

        // Batasi maksimal 3 item ditampilkan, sisanya diganti dengan "..."
        if (selectedValues.length === 0) {
            dropdownButton.textContent = 'Pilih Pihak Berkepentingan';
        } else if (selectedValues.length <= 5) {
            dropdownButton.textContent = selectedValues.join(', ');
        } else {
            dropdownButton.textContent = selectedValues.slice(0, 5).join(', ') + ', ...';
        }
    }
...
document.getElementById('kriteriaSelect').addEventListener('change', updateSeverityDropdown);
 ...
 <div class="row mb-3">
    ...
    <div class="card">
    <span class="badge bg-info" style="font-size: 1.25rem;">Tindakan Lanjut </span>
    </div>
...
document.getElementById('addMore').addEventListener('click', function() {
        ...
        var newInputSection = `
            <div class="dynamic-input mb-3">
                ...
                <button type="button" class="btn btn-danger btn-sm deleteSection ms-auto" title="Hapus">
