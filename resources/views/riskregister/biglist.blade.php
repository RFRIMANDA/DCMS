        ...
        <div class="modal-content">
            ...
            <form method="GET" action="{{ route('riskregister.biglist') }}">
                <div class="container py-3">
                    <div class="row mb-3">
                        @if(auth()->user()->role === 'admin' || auth()->user()->role === 'manajemen' || auth()->user()->role === 'supervisor')
                            <div class="col-md-6">
                                <div class="mb-3">
                        ...
                        @foreach ($formattedData as $data)
                            ...
                            <td>
                                <div class="btn-group" role="group">
                                    <!-- Edit Button -->
                                    <form action="{{ route('riskregister.edit', $data['id']) }}" method="get" style="margin: 0;">
                                        <button type="submit" title="Edit Issue" class="btn btn-success btn-sm me-1">
                                            <i class="bx bx-edit"></i>
                                        </button>
                                    </form>

                                    <!-- Delete Button -->
                                    <form id="delete-form-{{ $data['id'] }}" action="{{ route('riskregister.destroy', $data['id']) }}" method="POST" style="margin: 0;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" title="Hapus Issue" class="btn btn-danger btn-sm" onclick="confirmDelete({{ $data['id'] }})">
